...
define('WGS84_MAJOR_AXIS', 6378137.000);
define('WGS84_FLATTENING', 1.0 / 298.257223563);

# set defaults for Britain
$ellipsoid_shapes = array(
    'WGS84'=> array( 6378137.0000, 6356752.31425 ),
    'ETRS89' => array( 6378137.0000, 6356752.31425 ),
    'ETRN89' => array( 6378137.0000, 6356752.31425 ),
    'GRS80'  => array( 6378137.0000, 6356752.31425 ),
    'OSGB36' => array( 6377563.396,  6356256.910  ),
    'OSGM02' => array( 6377563.396,  6356256.910  ) );
# yes lots of synonyms

# constants for OSGB mercator projection
define('LAM0',RAD * -2.0);  # lon of grid origin
define('PHI0', RAD * 49.0);  # lat of grid origin
define('E0',  400000);   # Easting for origin
define('N0', -100000);   # Northing for origin
define('F0', 0.9996012717); # Convergence factor
/*
# useful patterns
...
#                        \Z}xo;
*/
function ll_to_grid($lat,$lon,$alt=0.0,$shape = 'WGS84')
{
    ...
    global $ellipsoid_shapes;
    list($a,$b) = $ellipsoid_shapes[$shape];
    ...
    $sp2  = pow(sin($phi),2.);
    $nu   = $a * F0 * pow((1. - $e2 * $sp2 ), -0.5);
    $rho  = $a * F0 * (1. - $e2) * pow((1. - $e2 * $sp2 ), -1.5);
    $eta2 = $nu/$rho - 1.;
    ...
    $I    = $M + N0;
    $II   = $nu/2.  * $sp * $cp;
    $III  = $nu/24. * $sp * pow($cp,3.) * (5.-$tp2+9.*$eta2);
    $IIIA = $nu/720.* $sp * pow($cp,5.) *(61.-58.*$tp2+$tp4);

    $IV   = $nu*$cp;
    $V    = $nu/6.   * pow($cp,3.) * ($nu/$rho-$tp2);
    $VI   = $nu/120. * pow($cp,5.) * (5.-18.*$tp2+$tp4+14.*$eta2-58.*$tp2*$eta2);

    $l = $lam - LAM0;
    $north = $I  + $II*pow($l,2.) + $III*pow($l,4.) + $IIIA*pow($l,6.);
    $east  = E0 + $IV*$l    +   $V*pow($l,3.) +   $VI*pow($l,5.);
...
function grid_to_ll($E,$N,$shape='WGS84')
{
    ...
    $VII  = $tp /   (2*$rho*$nu);
    $VIII = $tp /  (24*$rho*pow($nu,3)) *  (5 +  3*$tp2 + $eta2 - 9*$tp2*$eta2);
    $IX   = $tp / (720*$rho*pow($nu,5)) * (61 + 90*$tp2 + 45*$tp4);

    $sp = 1.0 / cos($phi) ;
    $tp6 = $tp4*$tp2;

    $X    = $sp/$nu;
    $XI   = $sp/(   6*pow($nu,3))*($nu/$rho + 2*$tp2);
    $XII  = $sp/( 120*pow($nu,5))*(      5 + 28*$tp2 +   24*$tp4);
    $XIIA = $sp/(5040*pow($nu,7))*(    61 + 662*$tp2 + 1320*$tp4 + 720*$tp6);

    $e = $E - E0;

    $phi = $phi        - $VII*pow($e,2) + $VIII*pow($e,4) -   $IX*pow($e,6);
    $lam = LAM0 + $X*$e -  $XI*pow($e,3) +  $XII*pow($e,5) - $XIIA*pow($e,7);
...
function _compute_M($phi, $b, $n)
{
    ...
    return $b * F0 * (
         ...
         - 35/24*pow($n,3)             * sin(3*$p_minus) * cos(3*$p_plus)
           );
}
/*
'''
